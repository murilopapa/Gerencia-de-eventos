package com.example.treinokotlin.mechanism.livedata
...
data class Resource<out DataType>(val status: Status, val message: String? = null,
    ...
    companion object {
        ...
        fun <DataType> success(data: DataType? = null,
                               status: Status): Resource<DataType> = Resource(status, data = data)

        fun <DataType> error(message: String? = null, errorCode: Int? = null,
                             errorMessageId: Int? = null, data: DataType? = null): Resource<DataType> = Resource(Status.ERROR,
